...
day,
usd_price,
AVG(usd_price) OVER (ORDER BY day ROWS BETWEEN 50 PRECEDING AND CURRENT ROW) AS moving_avg_usd_price
FROM cryptopunkdata;
...
SELECT 
EXTRACT(Day FROM day) AS day_of_week,
...
SELECT 
    ROUND(eth_price, -2) AS eth_range,
    COUNT(*) AS frequency
FROM cryptopunkdata
GROUP BY eth_range
ORDER BY eth_range;
...
-- 9. Return a unioned query that contains the highest price each NFT was bought
 -- for and a new column called status saying “highest” with a query that has the lowest price each NFT was bought
--  for and the status column saying “lowest”. The table should have a name column, a price column called price,
...
SELECT 
    ...
    EXTRACT(MONTH FROM utc_date) AS month,
    name,
    COUNT(*) AS sales_count,
...
SELECT 
    utc_date, 
    usd_price,
    AVG(usd_price) OVER (ORDER BY utc_date ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS daily_avg_usd_price
FROM cryptopunkdata;
